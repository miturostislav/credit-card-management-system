...
jest.mock('../src/paymentGatewayClient', () => {
  return {
    __esModule: true,
    credit: jest.fn(() => Promise[rejectTransaction ? 'reject' : 'resolve']()),
  };
});

const { findCreditCardByNumber, updateCreditLimit } = require('../src/creditCardsModel');
const { credit } = require('../src/paymentGatewayClient');
beforeEach(() => {
...
describe('Credit credit card tests', () => {
  test('Should respond with 200 HTTP status code and credit credit card successfully', async () => {
    const amount = 43;
    const cardNumber = '8764313490142324';
    const response = await request(app)
      .post(`/api/v1/credit_cards/${cardNumber}/credit`)
      .send({ amount });
    ...
    expect(credit).toHaveBeenCalledWith(cardNumber, amount);
    expect(credit.mock.calls.length).toBe(1);
  });
  ...
  test('Should respond with 404 HTTP status code because of credit card couldnt be found', async () => {
    ...
    expect(findCreditCardByNumber).toHaveBeenCalledWith(cardNumber, expect.anything());
    expect(findCreditCardByNumber.mock.calls.length).toBe(1);
    expect(credit.mock.calls.length).toBe(0);
  });
